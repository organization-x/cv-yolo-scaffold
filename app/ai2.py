...
def yolo_save_img(image, class_ids, boxes, labels, confidences, colors, file_path):
    ...
    for i, box in enumerate(boxes):
        ...
        #font_scale = get_optimal_font_scale(text, w)
        # set the rectangle background to white
        
        # set some text
        # get the width and height of the text box
        (text_width, text_height) = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, fontScale=font_scale, thickness=2)[0]
        if text_width > w:
            thickness = 1
            for scale in reversed(range(0, 12, 1)):
                (text_width, text_height) = cv2.getTextSize(text, fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=scale/10, thickness=1)[0]
                #print(new_width)
                if (text_width < w//1.2):
                    font_scale = scale/10
                    break
        ...
        box_coords = ((text_offset_x, text_offset_y), (text_offset_x + text_width + text_width // 10, text_offset_y - text_height - text_height // 2 ))
        cv2.rectangle(image, box_coords[0], box_coords[1], rectangle_bgr, cv2.FILLED)
        cv2.putText(image, text, (text_offset_x + text_width // 20, text_offset_y - text_height // 4), cv2.FONT_HERSHEY_SIMPLEX,
...
def yolo_show_img(image, class_ids, boxes, labels, confidences, colors):
    ...
    for i, box in enumerate(boxes):
        ...
        # set some text
        # get the width and height of the text box
        (text_width, text_height) = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, fontScale=font_scale, thickness=.5)[0]
        # set the text start position
        text_offset_x = x
        text_offset_y = y - 3 
        # make the coords of the box with a small padding of two pixels
